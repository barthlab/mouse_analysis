...
def style_axes(g, xlabel, ylabel, xlim, ylim, xmajmult, xminmult, ymajnum,
               yminnum, hspace=0.45, wspace=0.25, suptitle=None):
    ...
    ymaj = ticker.LinearLocator(numticks=ymajnum) if ymajnum != None else ymajnum
    ymin = ticker.LinearLocator(numticks=yminnum) if yminnum != None else yminnum
    
    if g.axes_dict: 
        for (name,ax) in g.axes_dict.items():
            style_axes_helper(ax, name, xlabel, ylabel, xlim, ylim, xmaj, xmin, ymaj, ymin)
    else:
...
def style_hr(g, xlabel, ylabel, xlim, ylim, hspace=0.45, wspace=0.25, 
             suptitle=None):
    style_axes(g, xlabel, ylabel, xlim, ylim, 12, 4, 5, 13, 
               hspace=hspace, wspace=wspace, suptitle=suptitle)

def style_ms(g, xlabel, ylabel, xlim, ylim, hspace=0.45, wspace=0.25, 
             suptitle=None):
    style_axes(g, xlabel, ylabel, xlim, ylim, 500, 200, 5, 13, hspace=hspace,
...
def plot_hr(data, x="Time (hr)", y="lick", color='k',
            xlim=[-48, 24], ylim=[-10, 10], 
            ylabel="", xlabel="Time (hr)", hspace=0.45, 
            wspace=0.25, suptitle='', **kwargs):
...
def plot_trial_bar(data, x="condition", y="trial no", aspect=1, col="Day", 
                   ...
                   hue=None, xlim=[-0.5, 2.5], ylim=[0, 600], xlabel=None, 
                   ylabel="Number of Trials", **kwargs):
    g = barplot(data, x=x, y=y, aspect=aspect, col=col, color=color, 
                palette=palette, hue_order=hue_order, hue=hue, 
                col_wrap=col_wrap, width=0.4, **kwargs)
    style_axes(g, xlabel, ylabel, xlim, ylim, None, None, 5, 13)
...
def plot_trial_hr(data, x="Time (hr)", y="trial no", col="condition", color='k', 
                  ...
                  hspace=0.45, wspace=0.25, suptitle=None, fill=True, **kwargs):
